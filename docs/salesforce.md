...
The Salesforce and language suites currently pass 1,019 tests (20 optional-language skips), including a parametrized identity/coverage contract for every registered type. This is a coverage contract, not a promise of complete semantics for all 533 types. Cross-object relationship binding uses precomputed parent/child schema indexes rather than scanning every field per reference. Permission record line indexing is linear in source size rather than repeatedly rescanning large captures.
